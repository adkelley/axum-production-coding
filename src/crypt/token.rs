...
pub fn validate_web_token(orig_token: &Token, salt: &str) -> Result<()> {
    let config = &config::config();
...
fn _validate_token_sign_and_exp(orig_token: &Token, salt: &str, key: &[u8]) -> Result<()> {
    // -- Validate signature.
...
mod tests {
    ...
    fn test_validate_web_token_ok() -> Result<()> {
        ...
        let token_key = &config::config().TOKEN_KEY;
